...
sess = K.get_session()
vehicle_index= [11]#[2,5,6,7]
vehicle_dict={11:'stop sign'}#{2:'car',5:'bus',6:'train',7:'truck',9:'traffic light',11:'stop sign'}
class_names= read_classes('./src/ppl_lover/src/yolo_coco_classes.txt')
anchors=read_anchors('./src/ppl_lover/src/yolo_anchors.txt')
yolo_model=load_model('./src/ppl_lover/src/tiny_yolo.h5')
yolo_outputs = yolo_head(yolo_model.output, anchors, len(class_names))
image_shape = np.float32(240), np.float32(320)
scores, boxes, classes = yolo_eval(yolo_outputs, image_shape=image_shape, score_threshold=.05)
...
if __name__ == "__main__":
    ...
    # start = time.time()
    # image = cv2.imread('/home/neymar/Pictures/Cardetec/detec5.png')
    # yolo_car_detection(image,colors)
    # print 'toneymaral run time ' + str(time.time() - start)
